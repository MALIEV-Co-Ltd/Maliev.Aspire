...
| `INT-009` | Authenticated Intranet employee creates a DeliveryService-backed delivery note from a real OrderService order, verifies carrier/tracking/customer/item state, transitions status to `InTransit` and `Delivered`, requests the delivery PDF queue, reads persisted BFF detail JSON, and verifies the delivered note is listed. The Make Studio delivery-status gate adds customer-visible QuoteEngine delivered status from a paid order. Remaining gaps are proof-of-delivery evidence upload, delivery PDF artifact download/content verification, notifications, partial deliveries, and low-permission negative checks. |
...
| `INT-028` | Partial after automated run | Later automated browser E2E creates two versions on one quotation, verifies version 1/version 2/current marker in the project quote tab, verifies version PDF artifact linkage, validates PDF bytes per version, and proves version 1/version 2 storage paths differ. | Need side-by-side comparison UI, non-current version permissions, and semantic PDF text/pixel validation per version. |
