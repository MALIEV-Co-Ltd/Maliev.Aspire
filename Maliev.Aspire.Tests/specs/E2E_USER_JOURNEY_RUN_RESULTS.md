...
| `dotnet test B:\maliev\Maliev.Aspire\Maliev.Aspire.Tests\Maliev.Aspire.Tests.csproj --filter "FullyQualifiedName~BrowserJourneyGateTests" -p:UseSharedCompilation=false -m:1 /nr:false --logger "console;verbosity=minimal"` | Passed: 23 tests |
| `dotnet test B:\maliev\Maliev.Aspire\Maliev.Aspire.Tests\Maliev.Aspire.Tests.csproj --filter "Tier=E2E" -p:UseSharedCompilation=false -m:1 /nr:false --logger "console;verbosity=minimal"` | Passed: 25 tests |
...
| `INT-003` | Partial after automated run | Later automated browser E2E verifies the created customer detail page renders the profile, company, addresses, and notes data; separate global-search automation verifies search propagation to customer navigation. | Need edit flows for profile/payment terms/addresses/documents and related workflow propagation. |
...
| `PROC-002` | Partial after automated run | Later automated browser E2E signs in as the Aspire automation employee, creates a supplier through `/api/v1/suppliers`, verifies the supplier is selectable in `/purchasing/new`, and uses it to create a supplier-backed PO. | Need supplier edit UI, supplier detail route assertions, duplicate/tax-id validation, and supplier audit/status checks. |
